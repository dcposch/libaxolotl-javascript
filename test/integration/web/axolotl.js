...
describe('axolotl integration web', function () {
  ...
  it('produces the expected output', function () {
    ...
    var Axolotl = window.axolotl
    var axol = new Axolotl({
    ...
    return axol.encryptMessage(state, new Uint8Array([1, 2, 3, 4]).buffer).then(function (result) {
      var actualBody = window.dcodeIO.ByteBuffer.wrap(result.body).toHex()
